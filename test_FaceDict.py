...
def align_and_save(img_path, save_path, save_input_path, save_param_path, upsample_scale=2):
    ...
    np.savetxt(save_param_path, tform2.params[0:2, :], fmt='%.3f')
...
def reverse_align(input_path, face_path, param_path, save_path, upsample_scale=2):
    ...
    inv_mask = cv2.warpAffine(
        mask, inv_M, (w*upsample_scale, h*upsample_scale))
    upsample_img = cv2.resize(input_img, (w*upsample_scale, h*upsample_scale))
    inv_mask_erosion_removeborder = cv2.erode(inv_mask, np.ones(
        (2 * upsample_scale, 2 * upsample_scale), np.uint8))  # to remove the black border
    ...
    inv_mask_center = cv2.erode(inv_mask_erosion_removeborder, np.ones(
...
if __name__ == '__main__':
    ...
    for i, ImgPath in enumerate(ImgPaths):
        ...
        np.savetxt(os.path.join(SaveLandmarkPath, SaveName),
                   preds[:, 0:2], fmt='%.3f')
